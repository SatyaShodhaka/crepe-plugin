package com.dsc.plugins.crepe.graphquery.model.block;
...
public abstract class SugiliteBlock implements Serializable {
    ...
    public static int REGULAR_OPERATION = 1, CONDITION = 2, FOR_EACH_LOOP = 3, RETURN_VALUE = 4, END_BLOCK = 5,
            STARTING_BLOCK = 6, SPECIAL_OPERATION = 8;
    ...
    public SugiliteBlock getNextBlockToRun() {
        if (nextBlock == null) {
            if (parentBlock != null) {
                // handle the "merge" of condition blocks - if the currentBlock has no
                // nextBlock, and has a parentBlock, should return the nextBlock of parentBlock
    ...
    // each "run" method should execute the task wrapped in the block, and call the
    // "run" method of the next block
}
