...
public class OntologyQueryFilter implements Serializable {
    ...
    /**
     * execute the filter on the result of an OntologyQuery to return a subset of
     * SugiliteEntity
     * 
    ...
    public Set<SugiliteEntity> filter(Set<SugiliteEntity> sugiliteEntities, UISnapshot uiSnapshot) {
        ...
        if (filterType == FilterType.EXISTS) {
            for (SugiliteEntity entity : sugiliteEntities) {
                Set<SugiliteTriple> allMatchedEntities = uiSnapshot.getSubjectPredicateTriplesMap()
                        .get(new AbstractMap.SimpleEntry<>(entity.getEntityId(), relation.getRelationId()));
            ...
            for (SugiliteEntity entity : sugiliteEntities) {
                ...
                Set<SugiliteTriple> allMatchedTriples = uiSnapshot.getSubjectPredicateTriplesMap()
                        .get(new AbstractMap.SimpleEntry<>(entity.getEntityId(), relation.getRelationId()));
    ...
    /*
     * public Set<SugiliteEntity> filter(Set<SugiliteEntity> sugiliteEntities,
     ...
     * if (filterType == FilterType.EXISTS) {
     * for (SugiliteEntity entity : sugiliteEntities) {
     * //get all the string entities that have either hasText or HAS_CHILD_TEXT
     * relation with the entity
     * Set<SugiliteEntity<String>> allStringEntities =
     * OntologyQueryUtils.getAllStringEntitiesWithHasTextAndHasChildTextRelations(
     * entity, uiSnapshot);
     * 
     ...
     * Set<SugiliteTriple> filteredTriples =
     * uiSnapshot.getSubjectPredicateTriplesMap().get(new
     * AbstractMap.SimpleEntry<>(stringEntity.getEntityId(),
     ...
     * }
     * 
     * else if (filterType == FilterType.ARG_MAX || filterType ==
     * FilterType.ARG_MIN){
     * List<Pair<SugiliteEntity, Comparable>> entityWithObjectValues = new
     * ArrayList<>();
     * 
     * for(SugiliteEntity entity : sugiliteEntities){
     * //get all the string entities that have either hasText or HAS_CHILD_TEXT
     ...
     * if (object.getEntityValue() instanceof Comparable) {
     * entityWithObjectValues.add(new Pair<>(entity, (Comparable)
     ...
     * 
     * //sort the entityWithObjectValues list
     * Collections.sort(entityWithObjectValues, new Comparator<Pair<SugiliteEntity,
     * Comparable>>() {
     * 
     * @Override
     * public int compare(Pair<SugiliteEntity, Comparable> o1, Pair<SugiliteEntity,
     * Comparable> o2) {
     * try{
     * return
     * Double.valueOf(o1.second.toString()).compareTo(Double.valueOf(o2.second.
     * toString()));
     * }
     * 
     * catch (Exception e){
     ...
     * }
     * } else if (filterType == FilterType.ARG_MAX) {
     * Comparable value = entityWithObjectValues.get(entityWithObjectValues.size() -
     * 1).second;
     * for(Pair<SugiliteEntity, Comparable> pair : entityWithObjectValues){
     * if(pair.second.compareTo(value) == 0){
     * result.add(pair.first);
