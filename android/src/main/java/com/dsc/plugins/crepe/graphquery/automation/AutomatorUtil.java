package com.dsc.plugins.crepe.graphquery.automation;

import static com.dsc.plugins.crepe.graphquery.Const.HOME_SCREEN_PACKAGE_NAMES;
...
import com.dsc.plugins.crepe.graphquery.model.variable.VariableValue;
...
public class AutomatorUtil {
    ...
    public static void killPackage(String packageName, ActivityManager am) {
        ...
        try {
            ...
            e.printStackTrace();
            // do nothing, likely this exception is caused by running Sugilite on a
            // non-rooted device
        }
    ...
    public static List<AccessibilityNodeInfo> preOrderTraverseSiblings(AccessibilityNodeInfo node) {
        ...
        for (int i = 0; i < numSibling; i++) {
            ...
            if (currSib.getClassName().toString().equals(node.getClassName().toString()) &&
                    nodeRect.contains(compRect) && compRect.contains(nodeRect))
                continue;
    ...
    public static AccessibilityNodeInfo customGetParent(AccessibilityNodeInfo child) {
        ...
        if (potentialParent == null)
            return null;
        if (isChild(child, potentialParent))
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (newPotentialParent == null)
                continue;
            if (isChild(child, newPotentialParent))
    ...
    public static String textVariableParse(String text, Set<String> variableSet,
            Map<String, VariableValue> variableValueMap) {
        ...
        for (Map.Entry<String, VariableValue> entry : variableValueMap.entrySet()) {
            ...
            if (entry.getValue().getVariableValue() instanceof String)
                currentText = currentText.replace("[" + entry.getKey() + "]",
                        (entry.getValue().getVariableValue().toString()));
        }
        return currentText;
    }
}
