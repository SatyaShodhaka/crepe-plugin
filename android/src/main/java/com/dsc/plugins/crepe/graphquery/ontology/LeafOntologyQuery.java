...
public class LeafOntologyQuery extends OntologyQuery {
    ...
    public BiFunction<SubjectEntityObjectEntityPair, UISnapshot, Boolean> getQueryFunction() {
    ...
    public boolean overallQueryFunction(SugiliteEntity currNode, UISnapshot graph) {
        ...
        if (object != null) {
            for (SugiliteSerializableEntity o : object) {
                try {
                    if (QueryFunction.apply(new SubjectEntityObjectEntityPair(currNode, new SugiliteEntity(o)),
                            graph)) {
                        objectBool = true;
                        break;
                    }
    ...
    public String toString() {
        ...
        // base case
        // this should have size 1 always, the array is only used in execution for when
        // there's a query whose results are used as the objects of the next one
