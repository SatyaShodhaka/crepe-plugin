...
public class OntologyQueryUtils {
    ...
    /**
     * consolidate nested conjs in queries (e.g. (conj xx (conj yy zz)) to (conj xx
     * yy zz))
     * 
     * @param query
     * @return
    ...
    public static OntologyQuery getQueryWithClassAndPackageConstraints(OntologyQuery query, Node clickedNode,
            boolean toAddClassQuery, boolean toAddPackageQuery, boolean toAddClickableQuery) {
    ...
    public static Set<SugiliteEntity<String>> getAllStringEntitiesWithHasTextAndHasChildTextRelations(
            SugiliteEntity entity, UISnapshot uiSnapshot) {
        Set<SugiliteEntity<String>> allStringEntities = new HashSet<>();
        Set<SugiliteTriple> hasTextRelationTriples = uiSnapshot.getSubjectPredicateTriplesMap()
                .get(new AbstractMap.SimpleEntry<>(entity.getEntityId(), SugiliteRelation.HAS_TEXT.getRelationId()));
        Set<SugiliteTriple> hasChildTextRelationTriples = uiSnapshot.getSubjectPredicateTriplesMap().get(
                new AbstractMap.SimpleEntry<>(entity.getEntityId(), SugiliteRelation.HAS_CHILD_TEXT.getRelationId()));
